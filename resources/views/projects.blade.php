 ...
 $(function(){
    ...
    <div style="margin-top:30px;max-width:100%;" class="row d-flex justify-content-center ">
        ...
        <div class="turningButtonContainer" style="width:300px;height:220px;">
            ...
            @if ($project->state == 0)
                <div class="turningButton"><i style="font-size:35px;margin-top:55px;color:#f9a602"class="icons far fa-dot-circle"></i><span style="font-size:30px;line-height:1.6">{{$project->ProjectTitle}}</span></div>
           @else
           ...
           @endif
            ...
            @if ($project->state == 0)
                <button onclick="location.href = '{{route('projectstate',['id'=>$project->id])}}'" style="padding:5px;margin-left:20px;width:40px;height:35px;"class="btn btn-light norms" data-toggle="tooltip" title="approve / deny projects"><i class="fas fa-align-justify"></i></button>
            @else
